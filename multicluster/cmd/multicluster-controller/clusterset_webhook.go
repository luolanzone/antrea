	...
	if req.Operation == admissionv1.Delete {
		if v.role == "leader" {
			...
			if len(mcaList.Items) > 0 {
				return admission.Errored(http.StatusPreconditionFailed, fmt.Errorf("failed to delete the ClusterSet %s since there are remaining member clusters connecting to the leader", req.Namespace+"/"+req.Name))
			}
		}
