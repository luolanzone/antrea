...
func TestOfCtl(t *testing.T) {
	...
	t.Run("Dump Table Flows With Filters", func(t *testing.T) {
		...
		DumpTableFlowsFiltered := func() ([]byte, error) {
			...
			prio := "priority=200"
			for i := 0; i < len(testDumpFlows); i++ {
				if !strings.Contains(testDumpFlows[i], table) || !strings.Contains(testDumpFlows[i], cookie) || !strings.Contains(testDumpFlows[i], prio) {
					continue
				}
				x = append(x, []byte(testDumpFlows[i])...)
				x = append(x, "\n"...)
		...
		match := fmt.Sprintf("table=3,cookie=0xa010000000000/0x%x,priority=200", ^uint64(0))
		mockOVSOfctlRunner.EXPECT().RunOfctlCmd("dump-flows", match, "--names").Return(DumpTableFlowsFiltered())
		filter := fmt.Sprintf("cookie=0xa010000000000/0x%x,priority=200", ^uint64(0))
