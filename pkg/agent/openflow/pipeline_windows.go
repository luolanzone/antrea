...
func (c *featurePodConnectivity) hostBridgeUplinkFlows(category cookie.Category, localSubnet net.IPNet) (flows []binding.Flow) {
	...
	flows = append(flows,
		// This generates the flow to forward ARP from uplink port to bridge local port.
		ARPSpoofGuardTable.ofTable.BuildFlow(priorityNormal).
			...
			Done(),
		// This generates the flow to forward ARP from bridge local port to uplink port.
		ARPSpoofGuardTable.ofTable.BuildFlow(priorityNormal).
			Cookie(c.cookieAllocator.Request(category).Raw()).
	...
	if c.networkConfig.NeedsDirectRoutingToPeer(peerIP, c.nodeConfig.NodeTransportIPv4Addr) && remoteGatewayMAC != nil {
		...
		flows = append(flows,
			...
			L3ForwardingTable.ofTable.BuildFlow(priorityNormal).
				...
				Action().NextTable().
				Done())
		flows = append(flows, c.l3FwdFlowToRemoteViaUplink(category, remoteGatewayMAC, *peerPodCIDR, false)...)
	} else {
		flows = append(flows, c.l3FwdFlowToRemoteViaGW(category, localGatewayMAC, *peerPodCIDR))
	}
	return flows
}
