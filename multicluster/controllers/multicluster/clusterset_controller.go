...
type ClusterSetReconciler struct {
	...
	LocalClusterManager clustermanager.LocalClusterManager
...
func (r *ClusterSetReconciler) createOrUpdateMultiClusterSetOnLeaderCluster(clusterSet *multiclusterv1alpha1.ClusterSet) {
	...
	if r.LocalClusterManager == nil {
		...
		r.LocalClusterManager = clustermanager.NewLocalClusterManager(r.Client, r.clusterID, clusterSet.GetNamespace(), r.Log)
	}
}
...
func (r *ClusterSetReconciler) updateMultiClusterSetOnMemberCluster(clusterSet *multiclusterv1alpha1.ClusterSet) error {
	...
	if r.RemoteClusterManager == nil {
		r.RemoteClusterManager = clustermanager.NewRemoteClusterManager(r.clusterSetID, r.Log, r.clusterID)
		go func() {
	...
	var removedLeaders map[common.ClusterID]clustermanager.RemoteCluster
	...
	for _, addedLeader := range addedLeaders {
		...
		go func(clusterID common.ClusterID, url string, secretName string) {
			...
			_, err := clustermanager.NewRemoteCluster(clusterID, r.clusterSetID, url, secretName, r.Scheme,
