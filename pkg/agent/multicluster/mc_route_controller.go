...
func NewMCDefaultRouteController(
	...
	if trafficEncryptionMode == config.TrafficEncryptionModeWireGuard {
		...
		controller.wireGuardConfig.MTU = controller.nodeConfig.NodeTransportInterfaceMTU - controller.networkConfig.MTUDeduction - config.WireGuardOverhead
	}
...
func (c *MCDefaultRouteController) syncWireGuard() error {
	...
	for _, ciImport := range ciImports {
		if err = c.updateSingleWireGuardPeer(ciImport); err != nil {
			updateErr = err
		}
	}
...
// updateSingleWireGuardPeer tries to update a WireGuard peer with ClusterInfoImport. If updating successfully,
// it will also create host route to WireGuard peer.
func (c *MCDefaultRouteController) updateSingleWireGuardPeer(ciImport *mcv1alpha1.ClusterInfoImport) error {
	...
	serviceIP, serviceCIDR, err := net.ParseCIDR(ciImport.Spec.ServiceCIDR)
	if err != nil {
		return err
	}
	...
	if err := c.wireGuardClient.UpdatePeer(ciImport.Name, ciImport.Spec.WireGuard.PublicKey, gatewayIP, allowedIPs); err != nil {
		klog.ErrorS(err, "Failed to update WireGuard peer", "ClusterInfoImport", klog.KObj(ciImport), "gatewayIP", gatewayIP, "allowedIPs", allowedIPs)
		return err
	}
	if err := c.addWireGuardRoute(remoteWireGuardIP.String()); err != nil {
		klog.ErrorS(err, "Failed to add WireGuard route on the host", "ClusterInfoImport", klog.KObj(ciImport))
		return err
	}
	return nil
}
...
// addWireGuardRoute initializes the WireGuard IP route on the host.
// If the route already exists, it will do nothing.
func (c *MCDefaultRouteController) addWireGuardRoute(remoteGatewayIP string) error {
	_, remoteGatewayIPCIDR, err := net.ParseCIDR(remoteGatewayIP + "/32")
	if err != nil {
		return err
	}
	klog.V(2).InfoS("Adding route on the host", "CIDR", remoteGatewayIPCIDR, "device", c.wireGuardConfig.Name)
	return c.routeClient.AddRouteForLink(remoteGatewayIPCIDR, c.wireGuardConfig.LinkIndex)
}

// cleanUpWireGuard deletes the WireGuard interface on the host.
// The WireGuard route will also be deleted automatically when the interfaces deleted.
...
func (c *MCDefaultRouteController) addMCFlowsForSingleCIImp(activeGW *mcv1alpha1.Gateway, ciImport *mcv1alpha1.ClusterInfoImport,
	...
	if installedCIImp != nil {
		oldTunnelPeerIPToRemoteGW := getPeerGatewayTunnelIP(installedCIImp.Spec, c.wireGuardInitialized)
...
// If WireGuard is enabled, the WireGuard interfaces use the first IP address of ServiceCIDR as IP
// address. So getPeerGatewayTunnelIP will return the first IP address of the ServiceCIDR as remote
// Gateway IP.
func getPeerGatewayTunnelIP(spec mcv1alpha1.ClusterInfo, enableWireGuard bool) net.IP {
	if enableWireGuard {
		...
		return serviceCIDR.IP.Mask(serviceCIDR.Mask)
	}
...
func getLocalGatewayIP(gateway *mcv1alpha1.Gateway, enableWireGuard bool) net.IP {
	if enableWireGuard {
		localGatewayIP, _, err := net.ParseCIDR(gateway.ServiceCIDR)
		if err != nil {
