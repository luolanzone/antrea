...
func TestStaleController_CleanUpService(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c := NewMemberStaleResCleanupController(fakeClient, common.TestScheme, "default", mcReconciler)
...
func TestStaleController_Reconcile(t *testing.T) {
	toDeleteSvcResExport := &mcv1alpha1.ResourceExport{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: "antrea-mcs",
			Name:      "cluster-a-default-nginx-service",
	...
	fakeClient := fake.NewClientBuilder().WithScheme(common.TestScheme).WithObjects(acnp1).Build()
	fakeRemoteClient := fake.NewClientBuilder().WithScheme(common.TestScheme).WithObjects(toDeleteSvcResExport).Build()
	commonarea := commonarea.NewFakeRemoteCommonArea(fakeRemoteClient, "leader-cluster", common.LocalClusterID, "antrea-mcs", nil)
	...
	c.Reconcile(ctx, reconcile.Request{
	...
	err := fakeRemoteClient.List(context.TODO(), resExpList, &client.ListOptions{})
	if err != nil {
		t.Errorf("Error when listing the ResourceExports after cleanup")
	}
	assert.Equal(t, 0, len(acnpList.Items), "failed to clean up stale ACNP")
	assert.Equal(t, 0, len(resExpList.Items), "failed to clean up stale ResourceExport")
}
