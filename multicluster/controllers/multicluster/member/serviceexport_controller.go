...
func (r *ServiceExportReconciler) updateSvcExportStatus(ctx context.Context, req ctrl.Request, cause reason) error {
	...
	newCondition := metav1.Condition{
		...
		// Keep the behavior as it is, we may revisit this when adding EndpointSlice support:
		/*  When updating ServiceExportStatus.Conditions, LastTransitionTime is always set to metav1.Now().
		    ...
		    Consider preserving the existing LastTransitionTime when Status is unchanged, or switching to
		    the standard helper (e.g. api/meta SetStatusCondition) which handles LastTransitionTime updates
		    correctly. Also, since metav1.Condition includes ObservedGeneration, it should be set
