...
type (
	...
	ServiceExportReconciler struct {
		...
		resourceExportsToRecreateMutex sync.RWMutex
		// resourceExportsToRecreate includes all existing ServiceExports' corresponding
		// Service and Endpoints types of ResourceExports which should be recreated in the leader cluster.
		resourceExportsToRecreate sets.Set[string]
	}
)
...
func (r *ServiceExportReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if svcInstalled {
		...
		if apiequality.Semantic.DeepEqual(svc.Spec.Ports, installedSvc.ports) {
			r.resourceExportsToRecreateMutex.RLock()
			if !r.resourceExportsToRecreate.Has(svcResExportName) {
			...
			r.resourceExportsToRecreateMutex.RUnlock()
		}
	}
	...
	if epsInstalled {
		...
		if apiequality.Semantic.DeepEqual(newSubsets, installedEp.subsets) {
			...
			r.resourceExportsToRecreateMutex.RLock()
			if !r.resourceExportsToRecreate.Has(epResExportName) {
				skipUpdateEPResourceExport = true
	...
	if !skipUpdateSvcResourceExport {
		...
		r.resourceExportsToRecreateMutex.Lock()
		if _, ok := r.resourceExportsToRecreate[svcExportNSName]; ok {
			klog.InfoS("change the flag to false", "resourceexport", epResExportName)
			r.resourceExportsToRecreate.Delete(svcExportNSName)
		}
		r.resourceExportsToRecreateMutex.Unlock()
	...
	if !skipUpdateEPResourceExport {
		...
		r.resourceExportsToRecreateMutex.Lock()
		if _, ok := r.resourceExportsToRecreate[epResExportName]; ok {
			klog.InfoS("change the flag to false", "resourceexport", epResExportName)
			r.resourceExportsToRecreate.Delete(epResExportName)
		}
		r.resourceExportsToRecreateMutex.Unlock()
	}
...
func (r *ServiceExportReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&source.Kind{Type: &mcv1alpha2.ClusterSet{}}, handler.EnqueueRequestsFromMapFunc(r.clusterSetMapFunc),
			builder.WithPredicates(predicate.GenerationChangedPredicate{})).WithEventFilter(versionChange).
		WithOptions(controller.Options{
			MaxConcurrentReconciles: common.DefaultWorkerCount,
		}).
		Complete(r)
...
func (r *ServiceExportReconciler) clusterSetMapFunc(a client.Object) []reconcile.Request {
	...
	if err == nil {
		...
		r.resourceExportsToRecreateMutex.Lock()
		defer r.resourceExportsToRecreateMutex.Unlock()
		for _, svcExport := range svcExports.Items {
			...
			svcResExportName := getResourceExportName(r.localClusterID, req, "service")
			epResExportName := getResourceExportName(r.localClusterID, req, "endpoints")
			r.resourceExportsToRecreate.Insert(svcResExportName)
			r.resourceExportsToRecreate.Insert(epResExportName)
		}
	}
	return requests
}
