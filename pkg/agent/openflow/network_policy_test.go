...
func setMockOFTables(ctrl *gomock.Controller, tableMap map[*FeatureTable]**mocks.MockTable) {
	for ft, mockTable := range tableMap {
		...
		table.EXPECT().GetNext().Return(ft.GetNext()).AnyTimes()
		table.EXPECT().GetMissAction().Return(ft.GetMissAction()).AnyTimes()
		table.EXPECT().GetName().Return("table").AnyTimes()
		tableCache.Update(ft)
		*mockTable = table // Update the value with generated mock table.
	}
}
