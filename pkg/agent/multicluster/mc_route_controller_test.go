...
func TestMCRouteControllerAsWireGuardGateway(t *testing.T) {
	...
	networkConfig := &config.NetworkConfig{}
	c, closeFn := newMCDefaultRouteController(t,
		&config.NodeConfig{
			Name: "node-4",
	...
	go func() {
		...
		_, cidr, _ := net.ParseCIDR("14.0.0.0/32")
		mockInterface.EXPECT().AddRouteForLink(cidr, 0).Times(1)
		c.processNextWorkItem()
...
func TestMCRouteControllerAsGateway(t *testing.T) {
	c, closeFn := newMCDefaultRouteController(
		t,
