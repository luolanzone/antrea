...
func (v *clusterClaimValidator) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	case admissionv1.Delete:
		...
		if len(clusterSetList.Items) > 0 {
			// ClusterSet webhook will guarantee that there is only one ClusterSet at most in a given Namespace.
			existingClusterSet = clusterSetList.Items[0]
			klog.InfoS("value", "value", clusterClaim.Value, "clsutersetname", existingClusterSet.Name, "req.name", req.Name)
		...
		if deny {
			klog.ErrorS(err, "Error deleting ClusterClaim, it's referred by a ClusterSet", "ClusterClaim", klog.KObj(clusterClaim), "ClusterSet", klog.KObj(&existingClusterSet))
