...
func (r *ResourceImportReconciler) handleResImpUpdateForService(ctx context.Context, resImp *multiclusterv1alpha1.ResourceImport) (ctrl.Result, error) {
	...
	svcName := common.GetNamespacedName(resImp.Spec.Namespace, resImp.Spec.Name)
	svc := &corev1.Service{}
	...
	if apierrors.IsNotFound(err) {
		...
		if err != nil {
			klog.Errorf("fail to create MCS Service %s/%s,err: %v", mcsObj.GetNamespace(), mcsObj.GetName(), err)
			return ctrl.Result{}, err
		}
...
func (r *ResourceImportReconciler) handleResImpUpdateForEndpoints(ctx context.Context, resImp *multiclusterv1alpha1.ResourceImport) (ctrl.Result, error) {
	...
	epName := common.GetNamespacedName(resImp.Spec.Namespace, resImp.Spec.Name)
	ep := &corev1.Endpoints{}
	...
	mcsEpObj := &corev1.Endpoints{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels:    map[string]string{common.AntreaMcsAutoGenLabel: ""},
		},
