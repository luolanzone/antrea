...
The `AntreaNodeConfig` feature gate (`Beta`, enabled by default) controls this
functionality. The CRD is included in the standard Antrea installation manifests.
...
- `secondaryNetwork.ovsBridges[]` - list of OVS bridge configurations (at most
  one bridge is supported). Each entry has:
  ...
  - `physicalInterfaces[]` - physical interfaces to connect to the bridge. Each
    ...
    - `allowedVLANs` - optional list of VLAN IDs or ranges (e.g. `"100"`,
      `"200-300"`) that are permitted on this interface as a trunk port. When
      set, the port is configured in trunk mode restricted to the listed VLANs.
      When omitted, all VLANs are allowed.
