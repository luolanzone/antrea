...
var (
	...
	// testNPMCMetricCookieRound0 is the OVS cookie prefix for NetworkPolicy metric flows at agent round 0
	// (same as cookie.NewAllocator(0).Request(cookie.NetworkPolicy).Raw()).
	testNPMCMetricCookieRound0 = "cookie=0x20000000000,"
...
func TestNetworkPolicyMetrics(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			flowFilter := fmt.Sprintf("cookie=0x%x/0x%x,priority=%d", npCookie, ^uint64(0), priorityNormal)
			gomock.InOrder(
				mockOVSClient.EXPECT().DumpTableFlows(EgressMetricTable.ofTable.GetID(), flowFilter).Return(tt.egressFlows, nil),
				mockOVSClient.EXPECT().DumpTableFlows(IngressMetricTable.ofTable.GetID(), flowFilter).Return(tt.ingressFlows, nil),
			)
